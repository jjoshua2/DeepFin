...
|-------|-------|-------|--------|---------------|
| 1 | Inventory | Hot paths, largest modules, duplicate orchestration | complete | Top simplify candidates are listed with owner files and risk. |
| 2 | Inference/worker | `inference.py`, `inference_threaded.py`, `worker.py`, `worker_pool.py`, `worker_assets.py` | processed | Model load/swap, compile/cache, queueing, and worker lifecycle paths have one clear contract each. |
| 3 | Server/distributed ops | `server/*`, `tune/distributed_runtime.py`, `tune/process_cleanup.py` | processed | Upload/lease/process cleanup logic has fewer duplicated state machines and clearer failure ownership. |
| 4 | Replay/shards | `replay/*`, `worker_buffer.py`, `tune/replay_exchange.py` | processed | Schema, optional arrays, staging, and sampling paths avoid duplicate compatibility logic. |
| 5 | Trainer/losses | `train/*`, `tune/trainable_*`, `trial_config.py` | processed | Loss-weight/config plumbing has one source of truth and fewer repeated metric/report fields. |
| 6 | Selfplay/finalization | `selfplay/*`, `arena.py`, `bench/play_batch_timing.py` | processed | Result labels, target construction, timeout/TB handling, and benchmark mirrors are not duplicated unnecessarily. |
| 7 | MCTS/UCI | `mcts/*`, `uci/*`, `tablebase.py` | processed | Python/C search paths and UCI runtime ownership are easier to reason about without weakening hot paths. |
| 8 | Scripts/docs/config | `scripts/*`, `configs/*`, docs | processed | Operational scripts share path/config helpers where useful and docs match the current workflow. |
| 9 | Remaining coverage | Everything not naturally owned by S001-S008 | complete | Every tracked source/config/script/test/doc file is assigned to a simplify track. |
...
| S007 | Search implementation parity | `chess_anti_engine/mcts/gumbel.py`, `gumbel_c.py`, `puct.py`, `puct_c.py`, `uci/search.py`, `uci/*`, `tablebase.py` | Python and C paths have overlapping terminal/root/legal-mask behavior. | Added an explicit cross-implementation gate for PUCT/Gumbel root masks and values, checked against independent root oracles for terminal/legal-move behavior, UCI TB/searchmoves behavior, and persistent root reuse; fixed the Gumbel C terminal-root value mismatch it exposed and simplified zero-root output handling. Broader hot-loop refactors still need this gate plus any refactor-specific parity tests. | `python3 -m pytest tests/test_mcts_uci_parity_gates.py tests/test_gumbel_root_many_edge_cases.py tests/test_mcts_c_tree.py tests/test_uci_searchmoves.py -q`; `ruff check chess_anti_engine/mcts/gumbel_c.py tests/test_mcts_uci_parity_gates.py`; `basedpyright chess_anti_engine/mcts/gumbel_c.py tests/test_mcts_uci_parity_gates.py` |
...
| S009 | Remaining file coverage | see "Coverage Matrix" | Some source areas were not changed by S001-S008 but still need explicit ownership. | Completed. The coverage matrix assigns every tracked source/config/script/test/doc file to a processed row with either a scoped simplification or an explicit defer decision. | Track-specific tests plus final lint/compile sweep. |
| S010 | Encoding and move indexing | `chess_anti_engine/encoding/*`, `chess_anti_engine/moves/*` | Encoding entrypoints repeated LC0 backend selection, extra-feature backend selection, and feature-dropout handling; move/CBoard parity code is hot and correctness-sensitive. | Implemented small shared helpers for LC0 plane selection, feature plane selection, and feature dropout in `encoding/encode.py`; added an independent CBoard/move-index parity harness for legal-index sets, mirrored policy-index semantics, fast `from_raw` construction, and `push_index()` state transitions across castling, en passant, promotions, terminal positions, and deterministic walks; centralized repeated Python policy-index/underpromotion math in `moves/encode.py`. Deferred C legal-generation refactors until a concrete cleanup needs this gate. | `python3 -m pytest tests/test_cboard_move_parity.py tests/test_encoding_basic.py tests/test_encode_optimization.py tests/test_move_encoding_lc0_4672.py tests/test_cboard_terminal.py tests/test_feature_dropout.py tests/test_mirror_augmentation.py -q`; `ruff check chess_anti_engine/encoding chess_anti_engine/moves tests/test_cboard_move_parity.py`; `basedpyright chess_anti_engine/encoding chess_anti_engine/moves tests/test_cboard_move_parity.py` |
...
| S013 | Utilities and run entrypoint | `chess_anti_engine/utils/*`, `run.py`, `config_keys.py`, `version.py`, package init | `run.py` and YAML config ownership are broad CLI/schema surfaces; utility hashing had a manual sentinel loop. | Simplified `sha256_file()` chunk iteration; deferred `run.py`/config schema reshaping because argparse destination compatibility and YAML allowlisting need a separate design. | `python3 -m pytest tests/test_atomic_io.py tests/test_worker_config_yaml.py tests/test_profile_distributed.py tests/test_trial_config.py tests/test_run_bootstrap.py -q`; `ruff check chess_anti_engine/utils chess_anti_engine/run.py chess_anti_engine/config_keys.py chess_anti_engine/version.py chess_anti_engine/__init__.py`; `basedpyright chess_anti_engine/utils chess_anti_engine/run.py chess_anti_engine/config_keys.py chess_anti_engine/version.py chess_anti_engine/__init__.py` |
| S014 | Tests | `tests/*` | Most tests intentionally keep local fixtures close to the adversarial scenario; selfplay tests duplicated the same uniform model and fake Stockfish process in four files. | Added `tests/selfplay_helpers.py` and reused it in selfplay/play-batch tests; left broader test helpers local unless duplication crossed multiple files with the same behavior. | `python3 -m pytest tests/test_selfplay_fraction.py tests/test_selfplay_sf_wdl_pov.py tests/test_selfplay_timeout_adjudication.py tests/test_play_batch_continuous.py -q`; `ruff check tests`; `python3 -m py_compile $(git ls-files 'tests/*.py')`; `basedpyright tests/selfplay_helpers.py tests/test_selfplay_fraction.py tests/test_selfplay_sf_wdl_pov.py tests/test_selfplay_timeout_adjudication.py tests/test_play_batch_continuous.py` |
| S015 | Server lease and tune ops final pass | `chess_anti_engine/server/*`, `chess_anti_engine/tune/*` | Lease JSON loading and published-trial scanning were duplicated; the larger upload, trainable, and distributed-runtime paths are guarded by durability, live-run, and status/reporting contracts. | Shared lease JSON-dict loading and published-trial discovery in `server/lease.py`; left upload compaction, trainable phase/report dicts, and worker launch orchestration unchanged after review. | `python3 -m pytest tests/test_server_trial_lease.py tests/test_server_upload_compaction.py tests/test_server_upload_durability.py tests/test_server_upload_security.py tests/test_tune_distributed_worker_cmd.py tests/test_trainable_config_ops.py tests/test_trainable_rng_checkpoint.py tests/test_tune_utils.py -q`; `ruff check chess_anti_engine/server chess_anti_engine/tune`; `basedpyright chess_anti_engine/server chess_anti_engine/tune` |
...
- [x] S009: inventory remaining package files not covered by S001-S008.

## Coverage Matrix

This matrix covers the 303 tracked repo files in scope for this pass
...
| S003/S005/S015 | `chess_anti_engine/server/**`, `chess_anti_engine/tune/**` | 23 | processed; lease discovery simplified, upload/trainable orchestration deferred |
...
| S014 | `tests/**` | 107 | processed; shared selfplay test fakes only |
...
|------|----|----------|--------|-----------|
| 2026-05-06 | S015 | Share server lease JSON loading and published-trial discovery; defer larger tune/server orchestration changes. | `load_lease`, lease pruning, active-run filtering, and active-prefix fallback repeated the same JSON-dict and manifest-scan logic. The larger upload and trainable paths still carry crash-recovery, reporting, worker-launch, and live-run contracts where extraction would need a design pass and broader golden tests. | Revisit upload/trainable extraction only with tests covering status/result field parity, restart recovery, duplicate upload retry, worker launch signatures, and active-run prefix migration. |
| 2026-05-06 | S009 | Add explicit coverage matrix for the whole tracked repo scope. | The simplify pass should not depend on memory of which directories were already examined. The matrix assigns all 300 tracked source/config/script/test/doc files to a track and now shows each row as processed or explicitly deferred. | Use the matrix as the durable checklist for future targeted simplify passes; do not reopen deferred hot paths without the named parity or golden tests. |
...
| 2026-05-06 | S013 | Keep run/config schema stable; simplify only the shared SHA-256 reader. | `run.py` has hundreds of CLI destinations that feed Tune config, YAML defaults, scripts, and live runs. A worthwhile simplification would be schema-driven argparse generation, but that needs compatibility tests first. The utility hash loop was safe to tighten now. | Before restructuring `run.py`, add a golden CLI/YAML/defaults test that compares representative `configs/pbt2_small.yaml` plus CLI overrides to the exact flat config consumed by `run_tune`. |
| 2026-05-06 | S014 | Share only repeated selfplay test fakes. | The test suite uses many file-local doubles that encode specific edge cases, and centralizing them would hide scenario intent. The repeated uniform policy/value model and fake Stockfish searcher had identical behavior across four selfplay/play-batch tests, so a helper removes real duplication without broad test coupling. | Keep adding shared test helpers only after the same fake appears in at least three files with identical behavior and focused tests still read clearly. |
| 2026-05-06 | S010 | Add an independent CBoard/move-index parity gate before touching hot-path move code, then centralize Python policy-index helpers. | The repeated LC0/feature/dropout branches were already simplified, but the move-policy LUTs, C legal move path, fast `from_raw` construction, CBoard push/index behavior, and C headers are high-blast-radius hot paths. The new gate checks CBoard legal-index sets, mirrored policy-index semantics, fast-constructor current-position fields, and `push_index()` state transitions against a local policy-index spec plus `python-chess` legal-state rules rather than treating the production Python move helpers as the oracle. With that gate in place, the Python move cleanup only centralizes repeated flat-index and underpromotion-plane assembly; C legal generation remains unchanged. | Keep `tests/test_cboard_move_parity.py` in the S010 verification set; broaden it before changing C legal generation internals or adding chess960/drop-move policy support. |
...
| 2026-05-06 | S007 | Add MCTS/UCI parity gate before hot-loop cleanup. | Root legal masks, single-legal roots, terminal roots, TB shortcuts, `searchmoves`, and persistent root reuse are the minimum safety net for future MCTS/UCI simplification. The gate does not assume Python search is the oracle; it checks both implementations against independent root contracts and then checks cross-implementation agreement. It found that Gumbel C returned the raw NN value for a checkmated terminal root. | Keep this gate in the required S007 verification set; broaden it before touching node accounting, tablebase-solved propagation, or deeper root reuse. |
| 2026-05-06 | S007 | Defer MCTS/UCI hot-loop simplification until parity tests are expanded. | The search surface spans Python and C Gumbel/PUCT, persistent UCI roots, tablebase overrides, legal masks, searchmoves, timing, stop/ponder state, and walker pools. Existing tests pin several edges, but a refactor without more parity coverage could silently change node accounting or root policy behavior. | Use the new root parity gate as the baseline, and add refactor-specific C/Python parity before extracting shared search helpers. |
