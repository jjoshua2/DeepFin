...
| 2026-05-06 | S010 | Add an independent CBoard/move-index parity gate before touching hot-path move code. | The repeated LC0/feature/dropout branches were already simplified, but the move-policy LUTs, C legal move path, CBoard push/index behavior, and C headers are high-blast-radius hot paths. The new gate checks CBoard legal-index sets and `push_index()` state transitions against a local policy-index spec plus `python-chess` legal-state rules rather than treating the production Python move helpers as the oracle. | Keep `tests/test_cboard_move_parity.py` in the S010 verification set; broaden it before changing mirrored policy mappings or C legal generation internals. |
