...
| S007 | Search implementation parity | `chess_anti_engine/mcts/gumbel.py`, `gumbel_c.py`, `puct.py`, `puct_c.py`, `uci/search.py`, `uci/*`, `tablebase.py` | Python and C paths have overlapping terminal/root/legal-mask behavior. | Added an explicit parity gate for PUCT/Gumbel C vs Python root masks, terminal roots, single-legal roots, TB root shortcuts, and `searchmoves`; fixed the Gumbel C terminal-root value mismatch it exposed. Broader hot-loop refactors still need this gate plus any refactor-specific parity tests. | `python3 -m pytest tests/test_mcts_uci_parity_gates.py tests/test_gumbel_root_many_edge_cases.py tests/test_mcts_c_tree.py tests/test_uci_searchmoves.py -q`; `ruff check chess_anti_engine/mcts/gumbel_c.py tests/test_mcts_uci_parity_gates.py`; `basedpyright chess_anti_engine/mcts/gumbel_c.py tests/test_mcts_uci_parity_gates.py` |
...
| 2026-05-06 | S007 | Add MCTS/UCI parity gate before hot-loop cleanup. | Root legal masks, single-legal roots, terminal roots, TB shortcuts, and `searchmoves` are the minimum safety net for future MCTS/UCI simplification. The new gate found that Gumbel C returned the raw NN value for a checkmated terminal root, while Python returned the terminal value. | Keep this gate in the required S007 verification set; broaden it before touching node accounting, tablebase-solved propagation, or root reuse. |
