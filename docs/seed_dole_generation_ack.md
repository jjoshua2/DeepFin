...
* The winner's existing idempotent claim replay is the heartbeat and renews the
  durable lease.
...
* Trainable rearm writes `{training_iteration, grant_token}`. Under the same
  gate lock, the server consumes it only when the token is still current and
  its lease expired; then exactly one caller can mint a new token.
...
No `PROTOCOL_VERSION` bump is required for wire parsing: `ack_grant_token` is
optional and older servers ignore it. Exact-claim replay from an older worker
still heartbeats while revision is unchanged. Full protection across a
