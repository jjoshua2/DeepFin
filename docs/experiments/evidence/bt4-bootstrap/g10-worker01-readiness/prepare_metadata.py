...
OUT=PREP/'G10_common_worker01_readiness_v1'
...
def records(p,key,wanted):
 ...
 with p.open() as f:
  for line in f:
   ...
   found[n]=(o,line)
 return found
names=[f'w01-{i:05d}.jsonl.zst' for i in range(128)]
prior=[f'w00-{i:05d}.jsonl.zst' for i in range(192)]
prospective=names
...
for lane,oldsource in enumerate(oldplan['sources']):
 ...
 raw=records(src/'w00.progress.jsonl',lambda o:P(o.get('path','')).name,set(prior))
 raw.update(records(src/'w01.progress.jsonl',lambda o:P(o.get('path','')).name,set(names)))
 ...
 s.update(physical_rows=sum(x['rows'] for x in entries),shards=128,games=len(newgames),prior_w00_00000_00191_game_overlap=0,compressed_raw_bytes=sum(x['bytes'] for x in metadata),cpu_affinity=[lane*2,lane*2+1])
...
write(OUT/'prior_selection_review.json',{'status':'PASS_METADATA','all_prior_registered_shards_worker00':True,'proofs':prior_proofs,'scope':'Pilot, original common batch, increment, parallel batch and both next-block attempts. Includes current selected batch independent of eventual completion.'})
