...
def successful_donor(plan):
 ...
 if receipt['status']!='PASS_EXPANDED50M_V50_CONTINUATION' or receipt['plan_sha256']!=plan['donor_plan_sha256']:raise ValueError('donor completion contract differs')
 ...
 if c!=summary['continuation'] or not summary['sampling']['complete'] or c['step_start']!=275896 or c['additional_epochs']!=1 or c['step_end']!=275896+summary['steps_realized'] or summary['sampling']['rows_realized']!=50548069 or summary['seed']!=105:raise ValueError('donor not final complete horizon')
...
def bind(plan):
 ...
 return {'donor':donor,'dataset_receipt':ref(plan['dataset_complete']),'union':d['union'],'schedule':d['schedule'],'roots':roots,'rows':50548069,'shards':6185,'sampling_seed':106,'prospective_seed':report['seed'],'prospective_batches':source_plan['batches_planned'],'scope':'Prospective101 proves corpus/recipe; actual106 sampler defines new order and realized optimizer budget. Full trainer history/value gates remain active.'}
