...
def main():
 ...
 try:
  ...
  s=json.loads((Path(p['out'])/'summary.json').read_text());r=s['continuation'];assert r['sha256']==bound['donor']['checkpoint']['sha256'] and r['step_start']==bound['donor']['step'] and r['additional_epochs']==1
  assert r['step_end']==r['step_start']+s['steps_realized'] and s['sampling']['complete']
  assert s['sampling']['rows_realized']==50548069 and s['sampling']['rows_planned']==50548069
  assert s['seed']==105 and s['sampling']['batches_realized']==s['steps_realized']
  ...
  guard();receipt.update(status='PASS_EXPANDED50M_V50_CONTINUATION',summary_sha256=sha(Path(p['out'])/'summary.json'),continuation=r,checkpoints=checkpoints,bound_inputs=bound,preserved_donor={'path':str(preserved),'sha256':sha(preserved)},actual_added_steps=s['steps_realized'])
  return 0
