...
def main():
 ...
 require(p['arm'] in ['A','B','C','D'],'invalid arm')
 ...
 targets=read(p['dataset_complete']);require(targets['status']=='COMPLETE_FACTORIAL58_TARGETS' and targets['rows']==58090688 and targets['shards']==7108,'target completion contract differs')
 require(targets['arms']['A']['roots']==p['base_roots'],'base35 partition/order differs')
 arm=targets['arms'][p['arm']];roots=arm['roots'];require(len(roots)==35 and len(set(roots))==35 and all(Path(x).is_dir() for x in roots),'requires35 distinct existing roots')
 qualification=arm['qualification'];require((qualification is None)==(p['arm']=='A'),'overlay qualification is required only for changed arms')
 pins=[ref(p['dataset_complete'])]
 if qualification is not None:require(sha(qualification['path'])==qualification['sha256'],'overlay qualification changed');pins.append(qualification)
 anchor=Path(p['initial_anchor']);anchor_value=None
 if p['arm']!='A':anchor_value=initial(anchor);pins.append(ref(anchor))
 ...
 try:
  ...
  require(summary['seed']==121 and summary['batch_size']==512 and summary['steps_realized']>0,'realized training settings differ')
