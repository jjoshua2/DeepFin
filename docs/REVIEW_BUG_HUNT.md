...
- Finding F047 opened/fixed in this cycle: the new MCTS/UCI parity gate found
  that Gumbel C returned the raw NN root value for an already-checkmated root,
  while Python Gumbel returned the terminal value. Gumbel C now seeds terminal
  root values from `CBoard.terminal_value()`.
