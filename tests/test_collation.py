...
from chess_anti_engine.replay import dataset as dataset_mod
...
def test_collate_arrays_defers_float16_cast_until_transfer(monkeypatch) -> None:
    ...
    def recording_to_tensor(
        arr: np.ndarray,
        *,
        device: str,
        dtype: torch.dtype | None = None,
    ) -> torch.Tensor:
        seen.append((arr.shape, arr.dtype, dtype))
        return original_to_tensor(arr, device=device, dtype=dtype)

    monkeypatch.setattr(dataset_mod, "_to_tensor", recording_to_tensor)
    ...
    torch.testing.assert_close(
        batch["sf_policy_t"],
        torch.from_numpy(sf_policy.astype(np.float32)),
        rtol=0.0,
        atol=0.0,
    )
