...
@pytest.mark.parametrize('keys', [list(range(127)), [7] * 256, list(range(261))])
...
def test_engine_generated_chess_workloads_are_reproducible() -> None:
    ...
    for corpus in provenance['corpora']:
        ...
        assert corpus['observations'] <= 260
    ...
    for trace in traces:
        ...
        assert expected(trace).startswith('begin 10\n')
