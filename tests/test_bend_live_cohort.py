...
@pytest.mark.parametrize(("capacity", "physical"), [(4096,4096), (4097,8192), (8192,8192), (65536,65536)])
def test_live_arena_storage_report(capacity: int, physical: int) -> None:
    r = {**root(), "arena_capacity": capacity, "arena_physical_slots": physical, "used_nodes": 1}
    assert validate(transcript(r, work()))["roots"]["1:1"]["arena_capacity"] == capacity


@pytest.mark.parametrize(("capacity", "physical", "used"), [(0,4096,1), (65537,131072,1), (8192,4096,1), (4096,8192,1),
                                                  (4096,4096,4097), (True,4096,1), (8192,None,1)])
def test_live_arena_contradictions(capacity: object, physical: object, used: int) -> None:
    r = {**root(), "arena_capacity": capacity, "arena_physical_slots": physical, "used_nodes": used}
    with pytest.raises(ValueError, match=r"invalid|exceed"):
        validate(transcript(r, work()))
