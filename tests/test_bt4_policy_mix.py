...
def _write_global_audit_inputs(root: Path, monkeypatch: pytest.MonkeyPatch) -> list[str]:
    """A full-width ruler that prefers SF while BT4 assigns a real legal tail."""
    board = chess.Board()
    ...
    for index in range(6):
        key = f"global-{index}"
        audit.append({
            "key": key, "fen": board.fen(), "phase": index % 3, "source": index % 2,
            "multipv": [{"move": move, "cp": 100 if rank == 0 else 0}
                        for rank, move in enumerate(moves)],
            "wdl": [500, 300, 200], "nodes": 1_000_000, "depth": 20,
        })
        d9.append({
            "key": key, "depths": [{"depth": 9, "lines": [
                [rank + 1, move, 100 if rank == 0 else 0, None]
                for rank, move in enumerate(moves)
            ]}],
        })
        bt4.append({"key": key, "topk": [[move, (rank + 1) / 210]
                                         for rank, move in enumerate(moves)]})
    paths = [root / name for name in ("ruler.jsonl", "d9.jsonl", "bt4.jsonl")]
    for path, records in zip(paths, (audit, d9, bt4)):
        path.write_text("".join(json.dumps(row) + "\n" for row in records))
    # Only substitute the frozen fixture's identity. Parsing, hashing, scoring,
    # receipt generation and receipt validation all execute the real code.
    monkeypatch.setattr(tool, "AUDIT_RULER_CONTRACT", {
        **tool.AUDIT_RULER_CONTRACT,
        **{key: tool.file_sha256(path) for key, path in zip(
            ("audit_set_sha256", "d9_labels_sha256", "bt4_cache_sha256"), paths,
        )},
    })
    return ["--audit-set", str(paths[0]), "--d9-labels", str(paths[1]),
            "--bt4-cache", str(paths[2])]


@pytest.mark.parametrize("temperature", [1.0, 0.5])
def test_global_cli_descriptive_audit_materializes_full_distribution(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, temperature: float,
) -> None:
    source_dir, shard_path, original = _write_source(tmp_path)
    sidecar_dir = _write_sidecar(tmp_path, source_dir, shard_path)
    sidecar = zarr.open_group(str(sidecar_dir / shard_path.name), mode="a")
    priors = np.asarray([[0.1, 0.6, 0.3], [0.2, 0.3, 0.5]], dtype=np.float32)
    sidecar[tool.SIDECAR_POLICY_FIELD][:, :3] = priors
    record = tmp_path / "preregistration.md"
    record.write_text("Compare global BT4 against SF in matched training and search.\n")
    identity = {"path": str(record.resolve()), "sha256": tool.file_sha256(record)}
    audit_inputs = _write_global_audit_inputs(tmp_path, monkeypatch)
    treatment = ["--scope", "global", "--alpha", "0.2",
                 "--bt4-temperature", str(temperature)]
    descriptive = ["--sf-audit-mode", "descriptive", "--experiment-record", str(record)]
    receipt_path = tmp_path / "descriptive.json"
    assert tool.main(["audit", *audit_inputs, *treatment, *descriptive,
                      "--json", str(receipt_path)]) == 0
    receipt = json.loads(receipt_path.read_text())
    assert receipt["gate"]["verdict"] == "kill"  # The SF ruler really disagrees.
    assert receipt["gate"]["training_permitted"] is True
    assert receipt["treatment_invariants"]["mass_reference"] == "normalized_total_legal_mass"
    assert receipt["admission"]["experiment_record"] == identity
    assert all(row["source_candidate_count"] == 20 for row in receipt["per_position"])

    out = tmp_path / "global-mixed"
    mix_args = [
        "mix", "--shards", str(source_dir), "--sidecar", str(sidecar_dir),
        "--out", str(out), *treatment, "--expected-rows", "2", "--expected-shards", "1",
        "--expected-source-summary-sha256", tool.file_sha256(source_dir / tool.DERIVE_SUMMARY),
        "--audit-receipt", str(receipt_path),
    ]
    # A descriptive receipt cannot silently change a default-gated invocation.
    with pytest.raises(ValueError, match="admission"):
        tool.main(mix_args)
    assert not out.exists()
    assert tool.main([*mix_args, *descriptive]) == 0
    destination: Any = zarr.open_group(str(out / shard_path.name), mode="r")
    source = original["policy_target"][:, :3].astype(np.float64)
    source /= source.sum(axis=1, keepdims=True)
    tempered = priors.astype(np.float64) ** (1 / temperature)
    tempered /= tempered.sum(axis=1, keepdims=True)
    expected = (0.8 * source + 0.2 * tempered).astype(np.float32).astype(np.float16)
    actual = np.asarray(destination["policy_target"][:])
    assert np.array_equal(actual[:, :3], expected)
    assert actual[1, 2] > 0  # BT4 reaches a legal move with zero SF mass.
    assert np.all(actual[:, 3:] == 0)
    assert np.allclose(actual.astype(np.float64).sum(axis=1), 1, atol=np.finfo(np.float16).eps)
    source_group: Any = zarr.open_group(str(shard_path), mode="r")
    assert np.array_equal(source_group["policy_target"][:], original["policy_target"])
    for field in original:
        if field == "policy_target":
            continue
        assert np.array_equal(destination[field][:], original[field]), field
        for source_file in (shard_path / field).rglob("*"):
            if source_file.is_file():
                copy = out / shard_path.name / source_file.relative_to(shard_path)
                assert copy.read_bytes() == source_file.read_bytes(), source_file
    summary = json.loads((out / tool.MIX_SUMMARY).read_text())
    assert summary["admission"]["experiment_record"] == identity
    assert summary["selected_mass_abs_drift"]["reference"] == "normalized_total_legal_mass"

    # The old mode still banks the negative audit and refuses admission.
    gated = tmp_path / "gated.json"
    assert tool.main(["audit", *audit_inputs, *treatment, "--json", str(gated)]) == 2
    assert json.loads(gated.read_text())["gate"]["training_permitted"] is False
    # Changing the preregistration after audit invalidates the materializer.
    record.write_text("Changed after the audit.\n")
    with pytest.raises(ValueError, match="admission"):
        tool._load_audit_receipt(
            receipt_path, alpha=0.2, scope="global", bt4_temperature=temperature,
            near_max_ratio=0.5, sf_audit_mode="descriptive", experiment_record=record,
        )


@pytest.mark.parametrize(("mode", "record"), [("descriptive", None), ("gate", "record")])
def test_sf_audit_mode_requires_explicit_record_binding(
    tmp_path: Path, mode: str, record: str | None,
) -> None:
    with pytest.raises(ValueError, match="requires"):
        tool._audit_admission(mode, tmp_path / record if record else None)


def test_descriptive_audit_cannot_admit_failed_fidelity() -> None:
    assert not tool.audit_training_permitted(
        verdict="kill", treatment_invariants_passed=False, sf_audit_mode="descriptive",
    )


def test_descriptive_cli_audit_still_refuses_lost_policy_mass(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    audit_inputs = _write_global_audit_inputs(tmp_path, monkeypatch)
    record = tmp_path / "preregistration.md"
    record.write_text("Judge playing strength independently of SF agreement.\n")
    original_mix = tool.mix_policy_targets

    def lose_mass(*args: Any, **kwargs: Any) -> np.ndarray:
        return 0.5 * original_mix(*args, **kwargs)

    monkeypatch.setattr(tool, "mix_policy_targets", lose_mass)
    receipt_path = tmp_path / "lost-mass.json"
    assert tool.main([
        "audit", *audit_inputs, "--scope", "global", "--alpha", "0.2",
        "--sf-audit-mode", "descriptive", "--experiment-record", str(record),
