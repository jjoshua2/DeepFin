...
def test_aggregate_pgn_scores_from_candidate_perspective() -> None:
    ...
    assert r.cand_wins == 1 and r.draws == 1 and r.cand_losses == 0
...
def test_aggregate_pgn_attributes_time_flags_to_the_right_side() -> None:
    ...
    assert r1.candidate_flags == 1 and r1.baseline_flags == 0
    ...
    assert r2.baseline_flags == 1 and r2.candidate_flags == 0
...
def test_validation_gate_and_elo() -> None:
    ...
    assert safe.flag_safe and safe.elo is None
    ...
    assert "FAIL" in summary and "FLAGGED" in summary
