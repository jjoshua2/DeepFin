...
def test_a_NON_FINITE_or_out_of_range_key_cannot_poison_the_loss(bad: float) -> None:
    ...
    ⚑ THE INFINITIES ARE HERE FOR A DIFFERENT REASON, AND THIS TEST CANNOT SEE IT.
    They never threatened finiteness -- a clamp turns them into 1.0 and 0.0, which
    are perfectly finite -- so this assertion passed on them before and after the
    guard was widened. What they threatened is the DIRECTION of the fallback, and
    that needs
    `test_a_non_finite_gate_value_falls_back_to_off_not_to_maximum_on` below. Two
    tests because it is two claims: "cannot crash" and "cannot silently arm"."""
...
def test_a_non_finite_gate_value_falls_back_to_off_not_to_maximum_on() -> None:
    """⚑⚑ `.inf` IS NOT NaN, AND THE CLAMP SENDS IT TO THE WRONG END.

    The guard used to be `math.isnan`, next to a comment promising "non-finite
    falls back to the OFF value". Both infinities slipped past it into
    `min(max(v, lo), hi)` and landed on a RANGE ENDPOINT -- and for both keys the
    endpoint an infinity reaches is the MAXIMUM-ON value, not the off value:

      * `listed_mass_min: inf`   -> clamps to 1.0 -> gates every row with a tail
    ...
    `listed_mass_min=0.5`, which is checked first. Without that, "scale is 1.0"
    could just mean the fixture is ungateable.
    """
    reg = _constant_tail_row().unsqueeze(0)
    legal = _legal().unsqueeze(0)
    tail_target = _onehot(len(REAL_REGRETS) + 1).unsqueeze(0)  # mass on the FILL

  # Non-degeneracy: at a real threshold this row IS gated, so a 1.0 below is a
  # statement about the fallback and not about the fixture.
    armed_scale, armed_gated = sf_regret_gate_scale(
        reg, tail_target, legal, listed_mass_min=0.5, unlisted_scale=0.0,
    )
    assert float(armed_scale[0]) == 0.0
    assert float(armed_gated[0]) == 1.0

    for bad in (float("inf"), float("-inf"), float("nan")):
        scale, gated = sf_regret_gate_scale(
            reg, tail_target, legal, listed_mass_min=bad, unlisted_scale=0.0,
        )
        assert float(scale[0]) == 1.0, (
            f"listed_mass_min={bad} armed the gate instead of disabling it"
        )
        assert float(gated[0]) == 0.0

      # And the other key: a non-finite SCALE must leave the term untouched on a
      # row the threshold selects, rather than suppressing it completely.
        scale, gated = sf_regret_gate_scale(
            reg, tail_target, legal, listed_mass_min=0.5, unlisted_scale=bad,
        )
        assert float(scale[0]) == 1.0, (
