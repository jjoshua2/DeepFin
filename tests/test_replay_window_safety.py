...
window, i.e. deletes shards, before the caller gets the object. Opening a live
window "to read it" therefore truncates it. ``read_only=True`` is the safe
opener.
"""
...
def test_window_eviction_of_symlinks_leaves_the_salvage_pool_intact(tmp_path: Path) -> None:
    ...
    and empty the pool here -- destroying a revert point while the run holds
    it open.
    """
...
def test_constructing_a_writable_buffer_evicts_shards(tmp_path: Path) -> None:
    ...
    change that quietly stops the constructor from enforcing the window would
    change training's window semantics, not just probe ergonomics.
    """
    window = tmp_path / "window"
    _make_pool(window, 5)
