...
def test_uci_loader_requires_complete_load_for_embedded_arch(tmp_path: Path) -> None:
    """Audit L12: the arena's own loader must not accept a partial load.

    Both sides of ``scripts/arena_standard.py`` come through here. When the
    model was built from the checkpoint's own ``arch`` there is no legitimate
    reason for a key to drop, and up to 50% of them could vanish behind a
    single stdout line before the catastrophic-load guard fired -- which is
    exactly how a lopsided Elo gets believed.
    """
    ckpt = _write_tiny_checkpoint(tmp_path, params=None, include_arch=True)
...
def test_uci_loader_complete_embedded_arch_load_still_succeeds(tmp_path: Path) -> None:
    """The default strictness must not disturb an ordinary, exact load."""
    ckpt = _write_tiny_checkpoint(tmp_path, params=None, include_arch=True)
...
def test_uci_loader_params_json_path_stays_tolerant(tmp_path: Path) -> None:
    ...
    exact load off it would break legitimate reads of older checkpoints.
    """
