...
def test_actor_skips_encoder_and_model_queue_for_confirmed_rule_draw(monkeypatch) -> None:
    ...
    sent: list[str] = []
    lines = iter(['board ' + ' '.join(map(str, words)), 'path 0',
                  *['action ' + str(k) for k in keys], 'end_eval'])
    peer.write.side_effect = sent.append
    peer.line.side_effect = lambda: next(lines)

    def expect(expected: str) -> None:
        assert next(lines) == expected
    peer.expect.side_effect = expect
    ...
    actor = Actor(peer, root, encoder, oracle, 0, 4)
    broker = Batcher(4, 146)
    broker.register(0, 1)
    actor.receive(f'eval 1 1 0 {len(keys)}', broker)
    assert sent[-1] == draw_reply(1, 1, 0)
