...
def delete_shard_path(path: str | Path) -> None:
    ...
    designated revert point that the live run may still need. ``shutil.rmtree``
    on a symlinked directory would follow it and delete the pool's contents;
    unlinking removes only the link. Measured 2026-07-26 (audit G10): 280 links
    already evicted from the live window, and ``swap_512x16_20260711`` still
    held all 815 shards. Collapsing this to a plain ``rmtree`` destroys a
    revert point mid-run — ``tests/test_replay_window_safety.py`` pins it.
    """
    p = Path(path)
    if p.is_symlink():
