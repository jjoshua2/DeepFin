...
def load_model_from_checkpoint(
    ...
    *,
    ...
    ``load_state_dict_tolerant`` reserves it for — the model then claims to
    match the file exactly, so any dropped key is a partially fresh-initialised
    net rather than tolerable architecture drift. This is the eval/serving
    loader that both sides of ``scripts/arena_standard.py`` go through, and up
    to 50% of keys could previously vanish behind a single stdout line before
    the catastrophic-load guard fired (audit invariant L12). Pass
    ``require_complete=False`` to deliberately load a checkpoint into a
    different architecture; pass True to demand an exact load even off the
    ``params.json`` path.
    """
