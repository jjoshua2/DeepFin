...
def main() -> None:
    ...
    def _handle_stop(_signum: int, _frame: object) -> None:  # skylos: ignore  # signal handler signature
        nonlocal stop
        stop = True
    ...
    try:
        while not stop:
            ...
            for idx, proc in enumerate(children):
                ...
                if stop:
                    break
                children[idx] = subprocess.Popen(commands[idx], cwd=str(Path(__file__).resolve().parents[1]))  # long-lived child process
    finally:
