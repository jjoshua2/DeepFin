...
class Smolgen(nn.Module):
  ...
  # x: (B,64,D) -> bias: (B,H,64,64)
        ...
        if relation_weight is not None:
            ...
            basis = self._buffers["relation_basis_flat"].to(dtype=bias.dtype, device=bias.device)
...
class ChessNet(nn.Module):
    ...
    def __init__(self, cfg: TransformerConfig):
        ...
        if self.input_global_embedding not in ("none", "bt4_board"):
  ...
  # Legacy repo mode: one smolgen bias computed once and reused by all layers.
    ...
    def _apply_deepnorm_init(self) -> None:
        ...
        with torch.no_grad():
            for module in self.blocks:
                ...
                if block.use_nla:
                    modules.extend(m for m in block.q_proj.net if isinstance(m, nn.Linear))
                    modules.extend(m for m in block.k_proj.net if isinstance(m, nn.Linear))
                    modules.extend(m for m in block.v_proj.net if isinstance(m, nn.Linear))
                elif block.qkv_projection == "split":
                    modules.append(block.q_proj)
                    modules.append(block.k_proj)
                    modules.append(block.v_proj)
                else:
    ...
    def _arc_attention_bias_for_layer(
        ...
        if weight is None:
            return None
        basis = self._buffers["arc_relation_basis_flat"].to(dtype=dtype, device=device)
