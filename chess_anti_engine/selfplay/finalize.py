...
def _update_aggregate_stats(
    ...
    *,
    ...
    # ...and the OTHER half: games that were persisted but could not be brought
    # back (`resume_discarded_games`). A discard happens at session start with
    # no game of its own to be counted against, so resume parks it on the state
    # and the first finalized game carries it out on the same path the success
    # takes. Drained, not copied: it must be counted exactly once.
