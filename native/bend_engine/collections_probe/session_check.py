...
from native.bend_engine.collections_probe.run_probe import check_compiler, command, session_notice
...
def build(source: Path, directory: Path, bun: str, cc: str) -> dict[str, Path]:
    ...
    result = subprocess.run([bun, str(source / 'bend2/main.ts'), str(sessions.HERE / 'main.bend'), '-o', str(generated)],
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix='collections-sessions-') as tmp:
        ...
        report = {'compiler_notice': (Path(tmp) / 'compiler-notice.txt').read_text(),
                  'original_sessions': sessions.verify(binaries, with_python_chess=True),
