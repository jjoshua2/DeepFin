...
revision="$("$bun" "$here/verify_compiler.js" --revision)"
repository="$("$bun" "$here/verify_compiler.js" --repository)"
# Revision-keyed defaults never silently reuse/reset yesterday's checkout.
compiler="${2:-$repo/build/bend_standalone_toolchain/$revision/source}"
if [[ -e "$output" ]]; then
...
{
  printf 'compiler_repository=%s\ncompiler_revision=%s\nmode=%s\n' "$repository" "$revision" "$mode"
  "$bun" "$here/verify_compiler.js" "$compiler"
