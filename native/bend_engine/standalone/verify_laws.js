...
function controls(compiler, temp) {
  ...
  variant("bypassed-reflection-predicate", d => replace(path.join(d, "proofs/Spec.bend"),
  ...
  variant("forged-table-certificate", d => fs.writeFileSync(path.join(d, "Check.bend"),
    "import Base\nimport ./proofs/Spec.bend as Spec\n"
    + "law forged: Spec.CheckedTables<0n, 0, 0>\n"
    + "def forged(): Spec.TableCertificate{{==}}\n"));
