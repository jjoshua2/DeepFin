...
def verify(command: list[str], checkpoint: Path, oracle_path: Path) -> dict[str, object]:
    ...
    def search(root: chess.Board, label: str) -> chess.Move | None:
        ...
        while i < len(text):
            ...
            if line.startswith('info string rule_draw '):
                fields = line.split()[3:]
                wanted = ref.next()
                assert wanted is not None and wanted == int(fields[0]), line
                ...
                sequence += 1
                assert seq == sequence and epoch == 1 and width == channels
                wanted = ref.next()
                assert wanted == node and request == ref.seq
                ...
                assert len(actions) == len(set(a[2] for a in actions)) > 0
                data: list[int] = []
                for plane in range(channels):
                    fields = text[i].split()
                    assert fields[:4] == ['info', 'string', 'neural_input', str(plane)] and len(fields) == 68
                ...
                for slot in range(1861):
                    fields = text[i].split()
                    assert fields[:4] == ['info', 'string', 'neural_logit', str(slot)] and len(fields) == 5
                    raw.append(int(fields[4]))
                    i += 1
                logits = floats(raw)
                with torch.inference_mode():
                    p, w = model(torch.from_numpy(tensor.copy()[None]))
                eager = np.concatenate((p[0].cpu().numpy(), w[0].cpu().numpy()))
                np.testing.assert_allclose(logits, eager, atol=2e-6, rtol=2e-5)
                error_logits = max(error_logits, float(np.abs(logits - eager).max()))
                fields = list(map(int, text[i].split()[3:]))
                assert text[i].startswith('info string neural_prob ') and len(fields) == 5
                status, wb, db, lb, count = fields
                assert status == 0 and count == len(actions)
                wdl = floats([wb, db, lb])
                i += 1
                priors_bits = []
    ...
    try:
        ...
        client.close()
    assert rows_checked > 30 and draws >= 2
