...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'deepfin-en_passant-laws-'));
const names = ['en_passant_make_move_exact_update', 'en_passant_make_move_preserves_representation'];
...
function copy(name) {
  ...
  return {e, s: path.join(e, 'standalone/proofs/en_passant'), chess: path.join(e, 'legal_probe/Chess.bend')};
}
...
try {
  ...
  console.error('PASS: two en-passant laws and importing consumer');
  reject('actual-captures-destination-not-remote','Actual.bend',d=>replace(d.chess,
    'cap = Bool.pick(U32, U32.is_eq(flag, 1), U32.xor(dst, 8), dst)',
    'cap = dst'), /Location: unfold\b/);
  reject('actual-remote-capture-wrong-rank','Actual.bend',d=>replace(d.chess,
    'U32.xor(dst, 8), dst)', 'U32.xor(dst, 16), dst)'), /Location: unfold\b/);
  reject('actual-ignores-flag-one','Actual.bend',d=>replace(d.chess,
    'cap = Bool.pick(U32, U32.is_eq(flag, 1)',
    'cap = Bool.pick(U32, U32.is_eq(flag, 3)'), /Location: unfold\b/);
  reject('actual-does-not-clear-target','Actual.bend',d=>replace(d.chess,
    'U64.or(target, select_u64(castle,',
    'U64.or(select_u64(U32.is_eq(flag,1),U64.zero(),target), select_u64(castle,'), /Location: unfold\b/);
  reject('actual-forces-king-for-en-passant','Actual.bend',d=>replace(d.chess,
    '+put = Bool.pick(U32, U32.is_zero(promotion), kind, promotion)',
    '+put = Bool.pick(U32, U32.is_eq(flag,1), 5, Bool.pick(U32, U32.is_zero(promotion), kind, promotion))'), /Location: unfold\b/);
  reject('actual-en-passant-does-not-toggle-turn','Actual.bend',d=>replace(d.chess,
    'U32.xor(side, 1), U32.and(get_rights(b), U32.not(rights_lost)), ep}',
    'Bool.pick(U32,U32.is_eq(flag,1),side,U32.xor(side,1)), U32.and(get_rights(b), U32.not(rights_lost)), ep}'), /Location: unfold\b/);
  reject('public-result-is-original-board','PROOF.bend',d=>replace(path.join(d.s,'LAWS.bend'),
    '== EP.expected(b,src,dst) : Chess.Board}', '== b : Chess.Board}'), /Location: LAWS\.en_passant_make_move_exact_update\b/);
  reject('omit-consistency-premise','PROOF.bend',d=>replace(path.join(d.s,'LAWS.bend'),
    'for good: {Board.valid(b) == True{} : Bool}',
    'for good: {True{} == True{} : Bool}'), /Location: LAWS\.en_passant_make_move_preserves_representation\b/);
  guard('missing-law', d => replace(path.join(d.s, 'LAWS.bend'), 'law en_passant_make_move_exact_update:', 'def missing:'), d => manifest(d.s), /./);
  guard('missing-proof', d => replace(path.join(d.s, 'PROOF.bend'), 'def Laws.en_passant_make_move_exact_update(', 'def missing('), d => manifest(d.s), /./);
  ...
  const report={focused_gate:'PASS',controls_gate:'PASS',consumer:'PASS',compiler_revision:PIN.revision,...identity,
    new_law_count:2,new_laws:names,inherited_gate_run:false,consumer_seconds:consumer.elapsed_seconds,
    negative_controls:controls,source_sha256s:before,
    scope:'Actual flag1/promotion0 complete Board update and partition preservation. Raw U32 squares and metadata; no legal-en-passant, source-pawn, rank/history, or king-safety theorem.'};
