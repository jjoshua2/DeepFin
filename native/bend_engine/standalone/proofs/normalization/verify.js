// Preserve the complete 74-law/126-control parent, then five address-normalization laws.
...
const parent=check('../prefix/verify.js',2700000);
assert.equal(parent.proof_gate,'PASS');assert.equal(parent.aggregate_law_count,74);assert.equal(parent.inherited_negative_controls,107);
...
assert.equal(focused.focused_gate,'PASS');assert.equal(focused.new_law_count,5);assert.equal(focused.negative_controls.length,14);
