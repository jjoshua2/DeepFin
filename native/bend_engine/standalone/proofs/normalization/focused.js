...
const temp=fs.mkdtempSync(path.join(os.tmpdir(),'deepfin-normalization-'));
const names=['bounded_mask_identity','bounded_mask_injective','bounded_read_direct','bounded_write_direct','bounded_separation_direct'];
...
function copy(name,base=false){const e=path.join(temp,name),r=path.join(e,'standalone');fs.mkdirSync(r,{recursive:true});fs.cpSync(path.join(root,'proofs'),path.join(r,'proofs'),{recursive:true});
 const d={e,r,s:path.join(r,'proofs/normalization'),cli};
...
try{
 ...
 const before=hashes(tracked);clean(invoke(path.join(suite,'consumer.bend')));console.error('PASS five laws and importing consumer');
 reject('address-mask-drops-bit16','Bound.bend',d=>replace(path.join(d.s,'Bound.bend'),'{U32.and(i,U32.sub(131072,1)) == i : U32}', '{U32.and(i,65535) == i : U32}'));
 reject('inclusive-bound-admits-capacity','Bound.bend',d=>replace(path.join(d.s,'Bound.bend'),'e: {U32.is_lt(i,131072) == True{} : Bool}','e: {U32.is_le(i,131072) == True{} : Bool}'));
 reject('widened-domain-admits-aliases','Bound.bend',d=>replace(path.join(d.s,'Bound.bend'),'e: {U32.is_lt(i,131072) == True{} : Bool}','e: {U32.is_lt(i,262144) == True{} : Bool}'));
 reject('actual-read-mask-off-by-one','Actual.bend',d=>replace(d.base,'Array.get.go(T, a, n, U32.and(i, U32.sub(n, 1)))','Array.get.go(T, a, n, U32.and(i, U32.sub(n, 2)))'),true);
 reject('actual-write-mask-off-by-one','Actual.bend',d=>replace(d.base,'Array.swap.go(T, a, n, U32.and(i, U32.sub(n, 1)), v)','Array.swap.go(T, a, n, U32.and(i, U32.sub(n, 2)), v)'),true);
 reject('separation-truncates-query','Actual.bend',d=>replace(path.join(d.r,'proofs/separation/Spec.bend'),
  'apart(t,n,U32.and(i,U32.sub(n,1)),U32.and(j,U32.sub(n,1)))','apart(t,n,U32.and(i,U32.sub(n,1)),U32.and(j,U32.shr(U32.sub(n,1))))'));
 guard('omitted-law',d=>replace(path.join(d.s,'LAWS.bend'),'law bounded_mask_identity:','def omitted:'),d=>manifest(d.s),/./);
 guard('omitted-proof',d=>replace(path.join(d.s,'PROOF.bend'),'def Laws.bounded_mask_identity(','def omitted('),d=>manifest(d.s),/./);
 ...
 guard('proof-hole',d=>fs.appendFileSync(path.join(d.s,'Bound.bend'),'\n?TODO\n'),d=>policy(path.join(d.s,'consumer.bend'),d.e),/proof hole/);
 guard('foreign-witness',d=>fs.appendFileSync(path.join(d.s,'Bound.bend'),'\nimport "./oracle.c"\n'),d=>policy(path.join(d.s,'consumer.bend'),d.e),/foreign/);
 guard('symlinked-proof',d=>{const f=path.join(d.s,'Bound.bend');fs.renameSync(f,f+'.original');fs.symlinkSync('Bound.bend.original',f);},d=>policy(path.join(d.s,'consumer.bend'),d.e),/nonregular/);
 const d=copy('unsafe-read');replace(path.join(d.s,'Actual.bend'),'def read(','@unsafe\ndef read(');
 ...
 controls.push({name:'unsafe-read',rejected:true,kind:'policy and exact output',raw_cli_status:warned.status});
 assert.equal(controls.length,14);assert.deepEqual(hashes(tracked),before);assert.deepEqual(verifyCompiler(compiler),identity);
 const report={focused_gate:'PASS',compiler_revision:PIN.revision,...identity,new_law_count:5,new_laws:names,inherited_gate_run:false,negative_controls:controls,consumer:'PASS',source_sha256s:before,
  scope:'All in-range depth-17 addresses retain identity; actual affine reads/writes and path predicates can use unmasked indices. Not distinct leaf paths or final table contents.'};
