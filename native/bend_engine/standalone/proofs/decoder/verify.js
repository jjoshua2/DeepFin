...
const parent = check('../board/verify.js', 25200000);
assert.equal(parent.proof_gate, 'PASS'); assert.equal(parent.aggregate_law_count, 123);
assert.equal(parent.inherited_negative_controls, 315); assert.equal(parent.negative_controls.length, 17);
...
assert.equal(result.consumer, 'PASS'); assert.equal(result.new_law_count, 4);
assert.equal(result.negative_controls.length, 18); assert.ok(result.negative_controls.every(x => x.rejected));
...
const report = {proof_gate: 'PASS', compiler_revision: result.compiler_revision, ...identity,
  aggregate_law_count: 127, new_law_count: 4, inherited_law_count: 123, inherited_negative_controls: 332,
