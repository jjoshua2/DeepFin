...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'deepfin-board-laws-'));
const names = ['empty_representation', 'initial_representation', 'fresh_put_preserves_representation',
  'put_preserves_metadata', 'metadata_preserves_representation', 'put_exact_bitboard_update'];
...
function copy(name) {
  ...
  return {e, s: path.join(e, 'standalone/proofs/board'), position: path.join(e, 'standalone/Position.bend')};
}
...
try {
  ...
  console.error('PASS: six representation laws and importing consumer');
  reject('actual-put-noop', 'Actual.bend', d => replace(d.position,
    '+bit = U64.bit(U32.to_nat(sq))', '+bit = U64.zero()'), /Location: bridge\b/);
  reject('actual-put-wrong-color', 'Actual.bend', d => replace(d.position,
    'U64.or(w, Chess.select_u64(white, bit, U64.zero()))', 'U64.or(w, bit)'), /Location: bridge\b/);
  reject('actual-put-wrong-kind', 'Actual.bend', d => replace(d.position,
    'Chess.select_u64(U32.is_eq(kind, 0), bit, U64.zero())', 'Chess.select_u64(U32.is_eq(kind, 1), bit, U64.zero())'), /Location: bridge\b/);
  reject('actual-put-changes-turn', 'Actual.bend', d => replace(d.position,
    'bit)), turn, rights, ep}', 'bit)), U32.xor(turn,1), rights, ep}'), /Location: bridge\b|Location: put_metadata\b/);
  reject('omit-freshness', 'Actual.bend', d => replace(path.join(d.s, 'Actual.bend'),
    'fresh: {S.fresh(b,sq) == True{} : Bool}', 'fresh: {True{} == True{} : Bool}'), /Location: insert\b/);
  reject('invalid-piece-tag', 'Rows.bend', d => replace(path.join(d.s, 'Spec.bend'),
    'case Pawn{}: 0', 'case Pawn{}: 6'), /Location: keep\b/);
  reject('corrupt-initial-board', 'PROOF.bend', d => replace(d.position,
    'U64.from_parts(0, 65535), U64.from_parts(4294901760, 0)',
    'U64.from_parts(0, 65535), U64.from_parts(4294901760, 1)'), /Location: LAWS\.initial_representation\b/);
  reject('metadata-destroys-piece-plane', 'Actual.bend', d => replace(d.position,
    'oldturn, oldrights, oldep} = b\n  Chess.Board{p, n, bi, r, q, k, w, bl, turn, rights, ep}',
    'oldturn, oldrights, oldep} = b\n  Chess.Board{U64.zero(), n, bi, r, q, k, w, bl, turn, rights, ep}'), /Location: metadata\b/);
  guard('missing-law', d => replace(path.join(d.s, 'LAWS.bend'), 'law empty_representation:', 'def missing:'), d => manifest(d.s), /./);
  guard('missing-proof', d => replace(path.join(d.s, 'PROOF.bend'), 'def Laws.empty_representation()', 'def missing()'), d => manifest(d.s), /./);
  ...
  guard('unsafe', d => replace(path.join(d.s, 'Actual.bend'), 'def insert(', '@unsafe\ndef insert('), d => graph(path.join(d.s, 'consumer.bend'), d.e), /unsafe dependency/);
  ...
  const result = {focused_gate: 'PASS', controls_gate: 'PASS', compiler_revision: PIN.revision, ...identity,
    new_law_count: 6, universal_laws: 4, closed_laws: 2, new_laws: names, inherited_gate_run: false,
    consumer: 'PASS', consumer_seconds: consumer.elapsed_seconds, negative_controls: controls, source_sha256s: before,
    scope: 'Board bitboard partition and exact fresh insertion; not king counts, metadata validity, occupied decoding, FEN reachability, or legal moves.'};
