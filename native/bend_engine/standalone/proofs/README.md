...
against the pinned Git objects, the universal bridge, 18 rejection controls,
...
The closed certificate is packaged as `Spec.CheckedTables<128n, 0, 512>`.
Its sole constructor requires a proof of `Spec.tables(128n, 0, 512) == True`;
there is no unchecked constructor or admitted fact. `Core.certified_table_slot`
and `Core.certified_tables` unpack that equality at generic indices. Reusing
the small indexed type avoids repeatedly normalizing the complete finite
predicate during type comparison. This is proof packaging, not weaker evidence.
The canonical proof still checks all eight conditions over every subset once.
...
A forged indexed table certificate must also fail: its constructor cannot
turn an unchecked false predicate into evidence. The pinned cyclic-template
false-proof regression is also rejected. Four policy
...
content/revision drift. These total 18 controls, with the rejecting layer named
