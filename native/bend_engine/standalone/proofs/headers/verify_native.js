// Opt-in complete-buffer headers/persistence checks on fresh distinct-cell storage.
...
const temp=fs.mkdtempSync(path.join(os.tmpdir(),'deepfin-header-native-'));
...
const paths=['standalone/Tables.bend','standalone/Subsets.bend','standalone/Text.bend',
  'standalone/proofs/headers/probe.bend','standalone/proofs/headers/verify_native.js'];
...
function reference(start,count,extra,extraStart,seed){
 const a=Array.from({length:131072},(_,i)=>seed^BigInt(i)),written=new Set(),blocks=[];let at=512;
 ...
 for(let key=0;key<start+count;key++){
  const rs=rays(key%64,key>=64),bits=rs.flatMap(r=>r.slice(0,-1)).sort((a,b)=>a<b?-1:1),mask=bits.reduce((x,b)=>x|b,0n),size=2**bits.length;
  if(key>=start){put(key,mask);put(128+key,BigInt(at));blocks.push({key,start:at,end:at+size,size});
   for(let i=0;i<size;i++)put(at+i,attacks(rs,deposit(i,bits)));}
  at+=size;
 }
 for(let s=extraStart;s<extraStart+extra;s++){put(256+s,leaper(s,knight));put(320+s,leaper(s,king));put(384+s,leaper(s,[[1,1],[-1,1]]));put(448+s,leaper(s,[[1,-1],[-1,-1]]));}
 return {text:a.map(half).join('\n')+'\n',blocks,end:at,written:written.size,untouched:a.length-written.size};
}
...
try{
 const cases=[
  [0,0,0,64,0xdeadbeef89abcdefn],
  [0,1,0,0,0xdeadbeef89abcdefn],
  [63,2,0,0,0x8000000000000001n],
  [127,1,64,0,0xffffffffffffffffn],
  [1,127,2,31,0x123456789abcdef0n],
  [0,64,0,0,0xfeedface76543210n],
  [64,64,64,0,0xa5a5a5a5a5a5a5a5n],
  [0,128,64,0,0n],
  [128,0,0,64,0xf000000080000000n]];
 const expected=cases.map(c=>reference(...c));assert.equal(expected[7].end,108160);assert.equal(expected[7].written,108160);
 ...
 for(const[mode,flags]of[['generic',[]],['portable',['-DBEND_U64_PORTABLE']],['native',['-march=native']],['ubsan',['-fsanitize=undefined','-fno-sanitize-recover=all']]]){
  ...
  const outputs=[];
  for(let i=0;i<cases.length;i++){const[k,n,en,ek,seed]=cases[i],actual=run(exe,['--threads','1',String(k),String(n),String(en),String(ek),...half(seed).split(' ')],180000);same(actual,expected[i].text);outputs.push(sha(actual));}
  const invalid=[['129','0','0','0','0','0'],['0','129','0','0','0','0'],['127','2','0','0','0','0'],
   ['0','0','65','0','0','0'],['0','0','2','63','0','0'],['0','0','0','65','0','0'],
   ['0','0','0','0','4294967296','0'],['x','0','0','0','0','0'],['0','1']];
  for(const x of invalid){const r=invoke(exe,['--threads','1',...x]);assert.equal(r.status,2);assert.match(r.stdout+r.stderr,/invalid header probe|expected six header/);}
 ...
 const report={native_gate:'PASS',compiler_revision:PIN.revision,...identity,
  ...
  logical_end:108160,allocation_capacity:131072,cases:cases.map(([start,count,extras,extra_start,seed],i)=>({start,count,extras,extra_start,seed:seed.toString(),end:expected[i].end,distinct_writes:expected[i].written,untouched_cells:expected[i].untouched})),
  modes,source_sha256s:before,cc:run(cc,['--version']).split('\n')[0],bun:process.versions.bun,
  scope:'Actual Tables.tables partial/suffix/full runs followed by actual extras on freshly unique-valued storage; every header and allocated cell compared. Candidate computes its own initial prefix from production masks. Not execution of proof schedules or independent source ray-refinement theorem'};
