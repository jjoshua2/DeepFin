...
const parent=check('../headers/verify.js',5400000);
assert.equal(parent.proof_gate,'PASS');assert.equal(parent.aggregate_law_count,103);
assert.equal(parent.inherited_negative_controls,226);assert.equal(parent.negative_controls.length,17);assert.ok(parent.negative_controls.every(x=>x.rejected));
console.error('PASS: unchanged 103-law parent and all 243 inherited rejection controls');
...
const report={proof_gate:'PASS',compiler_revision:p.compiler_revision,...identity,aggregate_law_count:106,new_law_count:3,inherited_law_count:103,
 inherited_negative_controls:243,new_laws:p.new_laws,negative_controls:p.negative_controls,
