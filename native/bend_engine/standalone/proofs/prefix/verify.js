// Preserve the complete 68-law/107-control parent, then check the new prefix gate.
...
const parent=check('../separation/verify.js',2100000);
assert.equal(parent.proof_gate,'PASS');assert.equal(parent.aggregate_law_count,68);assert.equal(parent.inherited_negative_controls,90);
assert.equal(parent.negative_controls.length,17);assert.ok(parent.negative_controls.every(c=>c.rejected));
console.error('PASS: unchanged 68-law parent and all 107 inherited controls');
...
const report={proof_gate:'PASS',compiler_revision:focused.compiler_revision,...identity,aggregate_law_count:74,new_law_count:6,
 new_laws:focused.new_laws,inherited_law_count:68,inherited_negative_controls:107,negative_controls:focused.negative_controls,
