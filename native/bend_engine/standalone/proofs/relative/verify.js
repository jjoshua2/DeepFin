// Preserve the 83-law parent; then execute all six new relative-address contracts.
...
const parent=check('../complete/verify.js',3000000);
assert.equal(parent.proof_gate,'PASS');assert.equal(parent.aggregate_law_count,83);assert.equal(parent.inherited_negative_controls,140);
assert.equal(parent.negative_controls.length,16);assert.ok(parent.negative_controls.every(c=>c.rejected));
console.error('PASS unchanged 83-law parent and 156 inherited controls');
const focused=check('./focused.js',1200000);
assert.equal(focused.focused_gate,'PASS');assert.equal(focused.new_law_count,6);assert.equal(focused.negative_controls.length,20);
assert.ok(focused.negative_controls.every(c=>c.rejected));assert.deepEqual(verifyCompiler(compiler),identity);
const report={proof_gate:'PASS',compiler_revision:focused.compiler_revision,...identity,aggregate_law_count:89,new_law_count:6,
 new_laws:focused.new_laws,inherited_law_count:83,inherited_negative_controls:156,negative_controls:focused.negative_controls,
