...
const parent = check('../ray/verify.js', 25200000);
assert.equal(parent.proof_gate, 'PASS'); assert.equal(parent.aggregate_law_count, 115);
assert.equal(parent.inherited_negative_controls, 276); assert.equal(parent.negative_controls.length, 19);
...
assert.equal(result.consumer, 'PASS'); assert.equal(result.new_law_count, 2);
assert.equal(result.negative_controls.length, 20); assert.ok(result.negative_controls.every(x => x.rejected));
...
const report = {proof_gate: 'PASS', compiler_revision: result.compiler_revision, ...identity,
  aggregate_law_count: 117, new_law_count: 2, inherited_law_count: 115, inherited_negative_controls: 295,
  new_laws: result.new_laws, negative_controls: result.negative_controls, source_sha256s: result.source_sha256s,
  inherited_source_sha256s: parent.source_sha256s, public_recipe: result.public_recipe, consumer: result.consumer,
  scope: result.scope};
