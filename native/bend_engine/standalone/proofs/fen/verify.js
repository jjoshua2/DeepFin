...
const parent=run('../frontier/verify.js',10800000);
assert.equal(parent.proof_gate,'PASS');assert.equal(parent.aggregate_law_count,141);
assert.equal(parent.inherited_negative_controls,384);assert.equal(parent.negative_controls.length,17);
assert.ok(parent.negative_controls.every(x=>x.rejected));
const focused=run('./focused.js',1800000);
assert.equal(focused.focused_gate,'PASS');assert.equal(focused.new_law_count,3);
...
const result={proof_gate:'PASS',aggregate_law_count:144,new_law_count:3,inherited_law_count:141,
 inherited_negative_controls:401,negative_controls:focused.negative_controls,
