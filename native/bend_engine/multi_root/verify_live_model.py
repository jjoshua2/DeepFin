...
def project(lines: list[str], identities: list[tuple[int, int]], batch: int,
    ...
    for line in lines:
        if line == OPEN:
            ...
            identity = (slot, gen)
            if identity not in expected or (slot in active and active[slot] not in retired):
            ...
            if active.get(slot) != (slot, gen) or (slot, gen) not in retired:
    ...
    if opened != 1 or seen != identities or retired != set(identities):
