...
class Client:
    def __init__(self, binary: Path, *, gate: bool = False, simulations: int = 1, fault: str = ""):
        self.events = self.release_fd = -1
        pass_fds: tuple[int, ...] = ()
        env = {**os.environ, "DEEPFIN_COHORT_ASYNC": "1"}
        if fault:
        ...
        self.proc = subprocess.Popen([str(binary), "--threads", "1", "--", str(simulations), "2", "0", "1"],
...
def arena_rejections(binary: Path) -> int:
    """The live entrypoint must validate capacity before opening its backend."""
    bad = ("", "0", "65537", "-1", "+1", "1.0", " 8192", "8192 ", "8192x", "１２３")
    with tempfile.TemporaryDirectory(prefix="live-arena-") as directory:
        for i, capacity_value in enumerate(bad):
            marker = Path(directory) / f"opened-{i}"
            env = {**os.environ, "DEEPFIN_COHORT_ASYNC": "1",
                   "DEEPFIN_COHORT_ARENA_NODES": capacity_value, "DEEPFIN_MULTI_TEST_OPEN": str(marker)}
            got = subprocess.run([str(binary), "--threads", "1", "--", "1", "2", "0", "0"],
                                 input="", capture_output=True, text=True, timeout=10, env=env, check=False)
            assert got.returncode == 2, (capacity_value, got.returncode, got.stderr)
            assert not marker.exists(), marker
            assert not got.stdout, got.stdout
    return len(bad)
...
def run(binary: Path, gate: Path, reference: Path) -> dict[str, Any]:
    return {"status": "passed", "scope": "compiled Bend live owner and actual worker with test-only callbacks; no neural-model or speed claim",
            ...
            "startup_rejections": startup_rejections(binary), "arena_rejections": arena_rejections(binary), "failures": [model_failure(gate, x) for x in ("fail", "nan")]}
