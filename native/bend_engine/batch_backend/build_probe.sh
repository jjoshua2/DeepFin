...
set -euo pipefail
if [[ $# != 4 ]]; then
  echo 'usage: build_probe.sh NEW_OUTPUT PACKAGE.pt2 LIBTORCH_CMAKE_PREFIX VERIFIED_COMPILER_SOURCE' >&2
  exit 2
fi
...
cmake -S "$single" -B "$output/build" -DCMAKE_BUILD_TYPE=Release \
  ...
  -DBEND_CONTRACT_DIR="$output" -DBEND_TARGET_NAME=deepfin-bend-batch-probe
