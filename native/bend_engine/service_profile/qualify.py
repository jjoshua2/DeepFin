...
def qualify(out: Path, cxx: str) -> dict[str, object]:
    ...
    for batch, channels, sanitize in [(b, c, False) for b in (1, 2, 4, 8, 16) for c in (146, 175)] + [
        ...
        nominal = subprocess.run(args, env=env, capture_output=True, text=True, timeout=30)
        ...
        for fault in ('startup', 'fail', 'throw', 'nonfinite', 'mismatch', 'tail'):
            child = subprocess.run(args, env={**env, 'SERVICE_TEST_FAULT': fault},
        ...
        for name in ('DEEPFIN_BEND_MODEL_TRACE', 'DEEPFIN_BEND_BUFFER_AUDIT'):
            child = subprocess.run(args, env={**env, name: '1'}, capture_output=True, text=True, timeout=30)
        ...
        for index, value in ((3, '0'), (3, '33'), (4, '1'), (4, '257'), (5, str(batch)), (3, '+1'), (4, '03')):
            bad = args.copy()
            bad[index] = value
            child = subprocess.run(bad, env=env, capture_output=True, text=True, timeout=30)
        ...
        for target in (x, y):
            ...
            for bad in (original[:-1], original + b'x', bytes.fromhex('0000c07f') + original[4:]):
                ...
                child = subprocess.run(args, env=env, capture_output=True, text=True, timeout=30)
                if child.returncode != 2 or child.stdout:
