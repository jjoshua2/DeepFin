...
def from_keys(name: str, keys: list[int]) -> tuple[list[Workload], Case]:
    ...
    if len(unique) < ENTRIES * 2 or len(keys) > WALKS * (PLIES + 1):
        raise ValueError(f'chess corpus {name}: {len(unique)} distinct / {len(keys)} observed; '
    ...
    # 2,048 buckets / 1,024-entry limit exceeds the 520-record bound without eviction.
    return results, Case('chess-' + name + '-encounters', 11, replay)
