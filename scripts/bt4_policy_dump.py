...
def run(args: argparse.Namespace) -> int:
    ...
    for key, fen in iter_rows(Path(args.rows)):
        ...
        if args.limit and len(keys) >= args.limit:
            break
    if done:
        print(f"[dump] resume: {len(done)} keys already present, {len(keys)} to do")
    if not keys:
    ...
    with out_path.open("a", encoding="utf-8") as fh:
        ...
        for start in range(0, len(keys), bs):
            ...
            usable = [(k, chess.Board(fen)) for k, fen in chunk]
            usable = [(k, b) for k, b in usable if any(b.legal_moves)]
            ...
            lines: list[str] = []
            for row, (key, board) in enumerate(usable):
            ...
            eta = f" eta {(len(keys) - done_n) / rate / 60:.1f} min" if rate > 0 else ""
            print(f"[dump] {done_n}/{len(keys)} {rate:.1f} pos/s{eta}", flush=True)
