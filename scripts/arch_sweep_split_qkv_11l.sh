...
watch_log="/tmp/offline_replay_epoch_arch_split_qkv_11l.log"
...
experiments=(
  # Is split Q/K/V neutral by itself?
  "arch_11l_splitqkv_mlp_only|runs/offline_replay_epoch_arch_11l_splitqkv_mlp_only_${stamp}|--seed 0 --qkv-projection split --candidates aurora_mlp_only"

  # Test the promising Ceres/Aurora-style scope: FFN + attention O/V, Q/K left on AdamW.
  "arch_11l_splitqkv_mlp_out_v|runs/offline_replay_epoch_arch_11l_splitqkv_mlp_out_v_${stamp}|--seed 0 --qkv-projection split --candidates aurora_mlp_out_v"
  "arch_11l_splitqkv_mlp_out_v_seed1|runs/offline_replay_epoch_arch_11l_splitqkv_mlp_out_v_seed1_${stamp}|--seed 1 --qkv-projection split --candidates aurora_mlp_out_v"

  # Bracket the scope: O only, all Q/K/V/O split, and fused all-attention baseline.
  "arch_11l_splitqkv_mlp_out|runs/offline_replay_epoch_arch_11l_splitqkv_mlp_out_${stamp}|--seed 0 --qkv-projection split --candidates aurora_mlp_out"
  "arch_11l_splitqkv_mlp_attn_all|runs/offline_replay_epoch_arch_11l_splitqkv_mlp_attn_all_${stamp}|--seed 0 --qkv-projection split --candidates aurora_mlp_attn_all"
  "arch_11l_fusedqkv_mlp_attn_all|runs/offline_replay_epoch_arch_11l_fusedqkv_mlp_attn_all_${stamp}|--seed 0 --qkv-projection fused --candidates aurora_mlp_attn_all"
)
...
log "starting split-QKV 11-layer queue: duration=${duration_s}s max_jobs=${max_jobs}"
start_s="$(date +%s)"
last_summary_s=0
