#!/usr/bin/env bash
# Wave-3 of the 2026-06-15 sidecar screen: PR#34 smolgen generator-size sweep.
# Defaults are hidden_channels=32, hidden_sz=256, gen_sz=256 — this brackets each
# knob up/down plus a combined bump and a shrink. Same window/seed/common-args as
# wave-1, so results compare against wave-1's `baseline`.
#
# Waits for the wave-2 driver (tmux 'sidecar_sweep_w2') to finish first; wave-2
# itself waits for wave-1, so this transitively runs last.
#
# Usage: scripts/sidecar_sweep_2026_06_15_wave3.sh {plan|run|summary}
set -euo pipefail
...
poll_s="${POLL_S:-30}"
stamp="20260615c"
out_base="runs/sidecar_sweep_20260615c"
sweep_log="${out_base}/sweep.log"
wait_driver="sidecar_sweep"   # wave-2 cancelled (redundant); chain directly off wave-1
ref_baseline="runs/sidecar_sweep_20260615/baseline/results.jsonl"
...
experiments=(
  "sg_gen384|--smolgen-gen-sz 384"
  "sg_gen512|--smolgen-gen-sz 512"
  "sg_hidden384|--smolgen-hidden-sz 384"
  "sg_ch48|--smolgen-hidden-channels 48"
  "sg_big|--smolgen-gen-sz 384 --smolgen-hidden-sz 384 --smolgen-hidden-channels 48"
  "sg_small|--smolgen-gen-sz 128 --smolgen-hidden-sz 128 --smolgen-hidden-channels 16"
)
...
print_plan() {
  echo "Sidecar sweep wave-3 (${stamp}) — PR#34 smolgen sizes — window=${window}"
  echo "Waits for '${wait_driver}' to finish, then runs ${max_jobs} at a time."
  local i=21 name extra
  for spec in "${experiments[@]}"; do
    IFS='|' read -r name extra <<<"${spec}"; printf "  %2d. %-14s %s\n" "${i}" "${name}" "${extra}"; i=$((i + 1))
  done
}
...
run_sweep() {
  ...
  if tmux has-session -t "${wait_driver}" 2>/dev/null; then
    log "waiting for '${wait_driver}' to finish before starting wave-3..."
    while tmux has-session -t "${wait_driver}" 2>/dev/null; do sleep 60; done
  fi
  log "=== wave-3 (${stamp}) start (max_jobs=${max_jobs}) ==="
  local -a pending=()
  ...
  while true; do
    ...
    pending=("${alive[@]}")
    (( ${#pending[@]} == 0 )) && { log "wave-3 all finished"; break; }
    local running=0
  ...
  log "=== wave-3 done; summarizing (ref = wave-1 baseline) ==="
  summarize || true
}
...
hdr = f"{'experiment':16s} " + " ".join(f"{k.replace('eval_',''):>14s}" for k in keys)
