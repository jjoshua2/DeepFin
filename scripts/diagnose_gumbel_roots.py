    ...
    for topk in topks:
        ...
        for sims in sims_list:
            ...
            for temp, rows in rows_by_temp.items():
                ...
                out[topk_key][f"sims_{sims}"][f"temp_{temp:g}"] = summary
            print(
                f"[{label}] topk={int(topk):>2} sims={sims:>3} n={summary.get('n', 0):>3} "
                f"search_regret={out[topk_key][f'sims_{sims}'][f'temp_0'].get('mean_regret', math.nan):.4f} "
                f"exp_regret@0.35={out[topk_key][f'sims_{sims}'].get('temp_0.35', {}).get('expected_regret', math.nan):.4f} "
                f"cat@0.35={out[topk_key][f'sims_{sims}'].get('temp_0.35', {}).get('catastrophic_prob_gt_010', math.nan):.3f} "
                f"dt={summary['seconds']:.1f}s"
            )
    del evaluator
    del model
    return out
