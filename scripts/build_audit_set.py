...
--replay-dir is required only for stage 1. To build a SECOND labeling of the
SAME positions (a referee pair: identical manifest, different Stockfish), copy
the existing ``<out>.manifest.jsonl`` next to the new ``--out`` and omit
``--replay-dir`` entirely -- stage 1 is skipped and nothing is re-sampled.
"""
...
def main() -> None:
    ...
    ap.add_argument("--replay-dir", type=Path, action="append", default=None,
                    help="replay shard directory (repeatable). Required only when "
                         "SAMPLING a new manifest; omit it when reusing an existing "
                         "<out>.manifest.jsonl (see the conditional check below).")
  ...
  # CONDITIONALLY required, not unconditionally: the manifest-reuse path below
  # never reads --replay-dir, and demanding it there forces the caller to pass a
  # value that is then silently ignored -- this codebase's signature defect,
  # sitting in the flag that guards it. Reuse is a first-class path (a referee
  # pair is the SAME positions with a different labeller, so re-sampling would
  # make the two referees incomparable), so it must not need a decorative arg.
    if not manifest_path.exists() and not args.replay_dir:
        raise SystemExit(
            f"--replay-dir is required to SAMPLE a new manifest ({manifest_path} "
            "does not exist). It is optional ONLY when an existing manifest is "
            "being reused."
        )
    ...
    def _write_readme(*, frozen: bool) -> None:
        readme_path.write_text(
            ...
            f"- replay dirs: {[str(d) for d in (args.replay_dir or [])]}"
            f"{'' if args.replay_dir else ' (manifest reused; not re-sampled)'}\n"
