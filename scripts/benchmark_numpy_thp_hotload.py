"""Held, child-only NumPy huge-page advice A/B on one pinned overlay loader.
...
The executed source-contrast runner and its receipts remain separate.
"""
...
NUMPY_ADVICE = {"control": "1", "candidate": "0"}
...
def read_kernel_thp_state() -> dict[str, str]:
    """Read the selected global THP modes; never change host settings."""
    state: dict[str, str] = {}
    for name in ("enabled", "defrag"):
        raw = (THP_SYSFS / name).read_text().strip()
        selected = re.findall(r"\[([^\]]+)\]", raw)
        if len(selected) != 1:
            raise Refused(f"cannot identify kernel THP {name} mode")
        state[name] = selected[0]
        state[f"{name}_raw"] = raw
    return state


def checked_kernel_thp_state(plan: dict[str, Any]) -> dict[str, str]:
    expected = plan.get("kernel_thp")
    if not isinstance(expected, dict) or expected.get("enabled") != "madvise":
        raise Refused("pilot requires pinned kernel THP enabled=madvise")
    realized = read_kernel_thp_state()
    if realized != expected:
        raise Refused(f"kernel THP state drift: expected {expected}, found {realized}")
    return realized
...
def check_pins(plan: dict[str, Any], *, runner: Path) -> None:
    ...
    if plan.get("numpy_madvise_hugepage") != NUMPY_ADVICE:
        raise Refused("NumPy advice arms must be control=1, candidate=0")
    if os.environ.get("MALLOC_ARENA_MAX") is not None:
        raise Refused("MALLOC_ARENA_MAX must remain unset in both arms")
    if sha256(runner) != plan["runner_sha256"]:
        raise Refused("runner pin mismatch")
    reference = plan["fixture_reference"]
    reference_path = Path(reference["path"])
    if sha256(reference_path) != reference["sha256"]:
        raise Refused("original eight-shard plan pin mismatch")
    original = load_json(reference_path)
    if (
        plan["fixture"] != original["fixture"]
        or plan["environment"] != original["environment"]
        or plan["sources"]["control"] != original["sources"]["candidate"]
        or plan["sources"]["candidate"] != original["sources"]["candidate"]
    ):
        raise Refused("THP arms must share the original candidate and fixture")
...
def child_environment(plan: dict[str, Any], arm: str) -> dict[str, str]:
    """One inherited environment with only the NumPy advice bit changed."""
    if plan.get("numpy_madvise_hugepage") != NUMPY_ADVICE or arm not in NUMPY_ADVICE:
        raise Refused("NumPy advice plan or arm changed")
    env = os.environ.copy()
    if env.get("MALLOC_ARENA_MAX") is not None:
        raise Refused("MALLOC_ARENA_MAX must remain unset in both arms")
    env["NUMPY_MADVISE_HUGEPAGE"] = NUMPY_ADVICE[arm]
    return env


def checked_numpy_advice(plan: dict[str, Any], arm: str, numpy: Any) -> bool:
    """Verify the child inherited the requested value and NumPy realized it."""
    requested = plan["numpy_madvise_hugepage"][arm]
    if os.environ.get("NUMPY_MADVISE_HUGEPAGE") != requested:
        raise Refused("child NumPy advice environment mismatch")
    realized = numpy.core.multiarray._get_madvise_hugepage()
    if type(realized) is not bool or realized is not (requested == "1"):
        raise Refused("child NumPy advice realization mismatch")
    return realized
...
def check_admission(plan: dict[str, Any], plan_path: Path, admission_path: Path) -> None:
    ...
    if admission.get("kernel_thp") != plan.get("kernel_thp"):
        raise Refused("admission kernel THP pin mismatch")
    checked_kernel_thp_state(plan)
...
def child(
    ...
    if os.environ.get("NUMPY_MADVISE_HUGEPAGE") != NUMPY_ADVICE[arm]:
        raise Refused("child NumPy advice was not set before import")
    source = Path(plan["sources"][arm]["path"])
    sys.path.insert(0, str(source))
    import numpy as np
    try:
        realized_advice = checked_numpy_advice(plan, arm, np)
    except Refused as exc:
        atomic_json(output_dir / f"arm_{index}.json", {
            "status": "REFUSED", "arm": arm, "index": index,
            "numpy_madvise_hugepage_env": os.environ.get("NUMPY_MADVISE_HUGEPAGE"),
            "error": str(exc),
        }, ARM_LIMIT)
        raise
    ...
    result: dict[str, Any] = {
        "arm": arm, "index": index, "status": "FAILED",
        "source": plan["sources"][arm],
        "numpy_madvise_hugepage_env": NUMPY_ADVICE[arm],
        "numpy_madvise_hugepage_realized": realized_advice,
        "kernel_thp_at_start": checked_kernel_thp_state(plan),
    }
    ...
    try:
        ...
        check_pins(plan, runner=Path(__file__))
        result["kernel_thp_at_end"] = checked_kernel_thp_state(plan)
        result.update({
...
def check_mode_receipt(receipt: dict[str, Any], plan: dict[str, Any], index: int) -> None:
    arm = ORDER[index]
    if receipt.get("index") != index or receipt.get("arm") != arm:
        raise Refused(f"arm {index} receipt identity mismatch")
    if (
        receipt.get("source") != plan["sources"][arm]
        or receipt.get("numpy_madvise_hugepage_env") != NUMPY_ADVICE[arm]
        or receipt.get("numpy_madvise_hugepage_realized") is not (arm == "control")
        or receipt.get("kernel_thp_at_start") != plan["kernel_thp"]
        or receipt.get("kernel_thp_at_end") != plan["kernel_thp"]
    ):
        raise Refused(f"arm {index} source, NumPy advice or kernel THP receipt mismatch")


def compare(receipts: list[dict[str, Any]], plan: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, receipt in enumerate(receipts):
        check_mode_receipt(receipt, plan, index)
    ...
    loads = [item["hot_load_seconds_per_shard"] for item in receipts]
    if any(
        len(times) != 8 or any(not math.isfinite(value) or value < 0 for value in times)
        for times in loads
    ):
        raise Refused("invalid per-shard hot-load timing")
    sums = [sum(times) for times in loads]
    control = statistics.median((sums[0], sums[3]))
    candidate = statistics.median((sums[1], sums[2]))
    if not math.isfinite(control) or control <= 0 or not math.isfinite(candidate):
        raise Refused("invalid hot-load timing")
    gain = 1 - candidate / control
    walls = [float(item["wall_seconds"]) for item in receipts]
    if any(not 0 < value <= plan["bounds"]["seconds_per_arm"] for value in walls):
        raise Refused("invalid whole-child wall timing")
    wall_a = statistics.median((walls[0], walls[3]))
    wall_b = statistics.median((walls[1], walls[2]))
    wall_regression = wall_b / wall_a - 1
    paired_directions = (sums[1] < sums[0], sums[2] < sums[3])
    screen_pass = gain >= 0.10 and all(paired_directions) and wall_regression <= 0.05
    return {
        ...
        "arm_whole_wall_seconds": walls,
        "median_control_whole_wall_seconds": wall_a,
        "median_candidate_whole_wall_seconds": wall_b,
        "fraction_whole_wall_regression": wall_regression,
        "adjacent_pair_improvements": list(paired_directions),
        "decision": (
            "WARRANTS_FUTURE_TRAINER_TEST" if screen_pass
            else "DOES_NOT_MEET_LOCAL_BENEFIT_GATE"
        ),
        "scope": "NumPy advice on eight B shards only; no trainer throughput or system-wide THP conclusion.",
    }
    ...
    try:
        ...
        for index, arm in enumerate(ORDER):
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True, env=child_environment(plan, arm),
            )
        ...
        check_pins(plan, runner=Path(__file__))
        summary.update(compare(receipts, plan))
