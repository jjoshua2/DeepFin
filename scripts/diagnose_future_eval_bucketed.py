...
def _fit_simplex(features: np.ndarray, target: np.ndarray, weights: np.ndarray | None = None) -> np.ndarray:
...
def _rmse(pred: np.ndarray, target: np.ndarray, weights: np.ndarray | None = None) -> float:
    if target.size == 0:
        return math.nan
    err = (pred - target) ** 2
    if weights is None:
