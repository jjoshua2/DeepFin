...
#
# ⚑ WHAT `search_shape` COSTS, RECORDED RATHER THAN DISCOVERED LATER. It spans
# `train_shape_stamp_fields()`, which appends `RUNNER_ARG_FIELDS` —
# `target_batch` and `vloss_weight`. So a `--target-batch 1024` vs `0` pair is
# now REFUSED, and that pair is a banked control someone deliberately produced.
# Taken knowingly: the failure on the other side is measured and worse (a
# pre-fix dump joins a post-fix one and reports a tight-CI delta that is
# entirely the ruler), and this repo has separately established that batching
# knobs ARE search-shape knobs — `--eval-max-batch` changes the realized search,
# so `target_batch` is not obviously the innocent bystander the earlier
# reasoning assumed. The `skip` set in the function below is per-METRIC scoping,
# NOT an operator escape hatch, so it does not recover this join; re-running the
# control under one shape does. If that control needs to join again, the fix is
# a declared per-field override with its own test, not widening the ruler.
