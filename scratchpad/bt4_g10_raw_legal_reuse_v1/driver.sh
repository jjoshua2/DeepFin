...
verify_code() {
  test "$(sha256sum "$OPS/verify_runtime.py" | cut -d ' ' -f1)" = "c2e170daca34b60bfa2e5f18bd4cae6d60e561f373805b4f9d2bdd686e7ce99a"
...
# Move first, then validate the exact request that was actually consumed.
test "$(sha256sum "$OPS/consume_handoff.py" | cut -d ' ' -f1)" = "ea20ed7050a29e4f782097e76461808b3a7fda8fa39a72864ad6febabf9f758c"
set +e
/usr/bin/python3 "$OPS/consume_handoff.py" "$OLD_OPS/pause.request" "$OPS/consumed_handoff" "$HANDOFF_SHA"
handoff_rc=$?
set -e
if (( handoff_rc == 3 )); then
  paused=1
  failed=0
  exit 0
elif (( handoff_rc != 0 )); then
  exit "$handoff_rc"
fi
