#!/usr/bin/env bash
# SF policy-teacher dose ladder — prereg 2026-08-19 + operational pins d2e07b96d.
# One driver invocation, 5 variants, then the audit yardstick on every arm.
set -uo pipefail
LIVE=/home/josh/projects/chess
WT=/home/josh/projects/chess-doseladder
OUT=$LIVE/scratchpad/dose_ladder
LOG=$LIVE/scratchpad/dose_ladder.log
mkdir -p "$OUT"
say() { printf '%s %s\n' "$(date '+%F %T')" "$*" >> "$LOG"; }

say "ladder start — 5 arms x 6000 steps from iter-595 over the salvage window"
