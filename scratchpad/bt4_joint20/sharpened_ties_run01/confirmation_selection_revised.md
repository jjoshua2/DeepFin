# Conditional fresh confirmation design — revised preparation
...
2. If the complete, qualified screen favors C or has an interval crossing zero,
   retain C as the provisional leader. An invalid, timed-out, unfinished or missing
   E0T05 bank stops selection and requires explicit assessment; it is not a loss.
    Choose the stronger plausible
...
Fix C as the reference in every confirmation branch. The candidate is the selected
E0T05 or G20T05 challenger. If development favored E0T05, a positive candidate
interval is the expected confirmation direction; if development favored C over
the chosen challenger, a negative interval is expected. An inconclusive development
contrast has no previously decisive direction to replicate.

Before registering the selected comparison, cheaply check the preserved fresh
bank's terminal EPDs against every completed bank used for selection, including
the eventual E0T05-versus-C bank. Require the recorded 500-pair development opening
alignment and zero overlap with the reserved confirmation bank. Reuse banked data;
this is not a corpus scan or training-purity certification.
...
inconclusive. Compare the fresh result with the selected development contrast. A fresh interval
wholly opposite a previously decisive development interval is contradictory
evidence. A fresh interval crossing zero is unresolved, not a demonstrated
reversal. If development was itself inconclusive, a decisive fresh interval adds
new evidence but cannot replicate or reverse a previously decisive finding.
Contradictory or unresolved evidence requires an explicit scientific decision;
no automatic retries, more games or further seeds are authorized by this draft. Preserve all results,
...
Revision note: addresses the four scientific clarifications in
`confirmation_selection_independent_review.json`, SHA256
`de1c9a2e3ed04206c5e909f9cff00b986b7220612ed6b46ec1ee8bd7d8b1b4e4`.
The original reviewed draft remains preserved. This revised preparation still
selects no recipe and authorizes no confirmation run.
