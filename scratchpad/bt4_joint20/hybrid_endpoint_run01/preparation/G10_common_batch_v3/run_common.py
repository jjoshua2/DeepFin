...
def freeze(args):
    ...
    for evidence in [read(REG)['previous_attempt'], *read(REG)['diagnostic_evidence']]:
        pins[evidence['path']] = evidence['sha256']
    for source in read(REG)['sources']:
        for key in ('source_manifest', 'receipt_snapshot', 'expected_policy_support_exclusions', 'expected_no_result_exclusions'):
            item = source[key]
...
def expected_exclusions(source, summary):
    def rows(item):
        require(sha(item['path']) == item['sha256'], 'exclusion census changed')
        return [json.loads(line) for line in Path(item['path']).read_text().splitlines()]

    support = rows(source['expected_policy_support_exclusions'])
    no_result = rows(source['expected_no_result_exclusions'])
    fields = ('source_dir', 'source_shard', 'source_row', 'worker_id', 'game_id', 'ply', 'input_key')
    def key(ref):
        return tuple(ref[field] for field in fields)
    expected = {key(ref): ref for ref in support}
    realized = summary['realized']
    actual = realized.get('policy_support_exclusions', [])
    require(len(expected) == len(support) == len(actual) == source['max_policy_support_misses']
            == realized.get('rows_dropped_policy_support'), 'support exclusion count differs')
    require(summary.get('max_policy_support_misses') == source['max_policy_support_misses'], 'support budget differs')
    require({key(ref) for ref in actual} == set(expected), 'actual support exclusions differ from census')
    for ref in actual:
        old = expected[key(ref)]
        require(ref['reason'] == 'selected_phase0_policy_support' and ref['policy_depth'] == 9
                and ref['full_history_input_key_verified'] is True
                and sorted(ref['missing_moves']) == sorted(old['missing_legal'])
                and sorted(ref['duplicate_moves']) == sorted(old['duplicate_moves']), 'exclusion proof differs')
    require(realized['rows_read'] == source['raw_rows'] and realized['rows_written'] == source['expected_emitted_rows']
            and realized['rows_dropped_envelope'] == 0
            and realized['rows_dropped_no_result'] == len(no_result) == source['expected_no_result_rows'], 'unexpected row accounting')
    excluded = {(ref['source_shard'], ref['source_row']) for ref in support + no_result}
    require(len(excluded) == len(support) + len(no_result), 'duplicate or overlapping exclusion references')
    require(all(ref['source_dir'] == source['source_dir'] for ref in support + no_result), 'foreign exclusion source')
    universe = {entry['source_shard']: entry['raw_rows'] for entry in source['selection']}
    require(sum(universe.values()) == source['raw_rows'], 'registered source row universe differs')
    require(all(name in universe and type(offset) is int and 0 <= offset < universe[name]
                for name, offset in excluded), 'exclusion is outside registered source rows')
    return excluded
...
def snapshot_derived(source):
    ...
    expected_exclusions(source, summary)
...
def qualify(source):
    ...
    excluded = expected_exclusions(source, summary)
    ...
    for path in paths:
        ...
        rows += count
    require(rows == n == source['expected_emitted_rows']
            and source['raw_rows'] - rows == len(excluded), 'survival/exclusion accounting differs')
...
def worker(plan):
    ...
    for source in reg['sources']:
        ...
        stage(name + '.derive', command(plan, 'derive_corpus_targets.py', '--corpus', source['source_dir'], '--out', source['derived_output'],
              '--limit', source['raw_rows'], '--max-policy-support-misses', source['max_policy_support_misses'],
              *reg['derive_common_args']), plan)
