"""Execute only the pinned cached-bank B100 audit; never materialize or train."""
import fcntl
import hashlib
import json
import os
...
BASE = STATE.parent / 'hybrid_endpoint_run01'
...
def main():
    ...
    if Path(plan['output']).exists():
        raise ValueError('audit output already exists; no adoption or retry')
    ...
    with (BASE / 'preparation.lock').open('a') as lock:
        ...
        record = {'schema': 1, 'stage': 'cached_B100_audit_only', 'status': 'STARTING',
        ...
        try:
            with (STATE / 'audit.log').open('x') as log:
                ...
                record['resource_usage'] = {'user_seconds': usage.ru_utime, 'system_seconds': usage.ru_stime, 'max_child_rss_kib': usage.ru_maxrss}
