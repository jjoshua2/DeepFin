#!/usr/bin/env python3
"""Fixed G20T1 COPY+VERIFY operation. No source deletion; default is plan-only.
...
ROOT = Path('/home/josh/projects/chess/scratchpad/bt4_joint20/G20T1_cold_archive_v1')
SOURCE = Path('/home/josh/projects/chess/data/nnue_derived/armB')
DEST = Path('/mnt/e/chess_derived_archive_20260908/G20T1_v1')
...
def mount_check(plan):
    result = subprocess.run(['/usr/bin/findmnt', '-J', '-T', '/mnt/e', '-o', 'TARGET,SOURCE,FSTYPE,OPTIONS'],
...
def resources(estimate=0):
    if (ROOT / 'STOP').exists() or (ROOT.parent / 'STOP').exists():
        raise RuntimeError('archive STOP requested')
    if DEADLINE is not None and time.time() >= DEADLINE - 30:
        raise RuntimeError('registered archive deadline reached')
    # Only a handful of this attempt's files: no source-tree size walk.
    staging = ROOT / 'staging' / 'qtemp_0.0005_hist_20m_bt4_global_G20T1'
    if staging.exists():
        size = sum(p.stat().st_size for p in staging.iterdir() if p.is_file())
        if size > 24 * GIB:
            raise RuntimeError('sampled staging-file ceiling exceeded')
...
def create_tar(names, archive, log, estimate, plan):
    global CHILD
    command = ['/usr/bin/tar', '--create', '--file=-', '--format=pax', '--numeric-owner', '--acls', '--xattrs',
    ...
    with log.open('xb') as errors, archive.open('xb') as output:
        CHILD = spawn(command, stdout=subprocess.PIPE, stderr=errors)
        child = CHILD
        try:
            while True:
...
def spawn(command, **kwargs):
    if DEADLINE is None:
        raise RuntimeError('missing owned absolute deadline')
    seconds = DEADLINE - time.time() - 30
    if seconds <= 0:
        raise RuntimeError('no archive child budget remains')
    # This independent timeout survives an abrupt driver death. Its process
    # group contains tar/rsync and descendants; outer timeout also caps driver.
    return subprocess.Popen(['/usr/bin/timeout', '--signal=TERM', '--kill-after=20s',
                             f'{seconds:.6f}s', *command], start_new_session=True, **kwargs)


def cleanup_child(grace=20):
    global CHILD, CLEANING
    if CHILD is None or CLEANING:
        return
    CLEANING = True
...
def sampled_guard(_signum, _frame):
    resources()
    signal.alarm(10)
...
def run_pool(item, plan, plan_sha):
    ...
    assert_stat(pool, item['source_root_stat'])
    ...
    try:
        ...
        receipt['rsync_command'] = ['/usr/bin/rsync', '--no-whole-file', '--partial-dir=.' + partial,
                                     '--bwlimit=16384', '--fsync', '--', str(archive), str(archive_destination)]
        with (attempt / 'rsync.log').open('xb') as log:
            CHILD = spawn(receipt['rsync_command'], stdout=log, stderr=subprocess.STDOUT)
            try:
...
def main():
    ...
    if not args.execute:
        return
    if args.deadline is None or not (time.time() + 30 < args.deadline <= time.time() + 14400):
        raise ValueError('execute requires finite registered deadline within four hours')
    ...
    signal.signal(signal.SIGALRM, sampled_guard)
    signal.alarm(10)
