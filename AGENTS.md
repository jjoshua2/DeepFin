...
- After pulling changes to `.c`/`.h` files, rebuild in place with `python3 setup.py build_ext --inplace` (NOT `pip install -e .` — the .venv setuptools lacks PEP 660). For the local production BF16 path use `CAE_EXT_NATIVE=1 CAE_EXT_LTO=1 python3 setup.py build_ext --inplace`; keep distributed wheels portable unless every target CPU matches. After a numpy upgrade add `--force`: stale cached `.so`s are silently reused.
