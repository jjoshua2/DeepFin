...
- `configs/pbt2_small.yaml` is the production config for active training (512-dim, 16-layer, 63,084,128 params). `configs/default.yaml` is the larger reference config (768-dim, 15-layer, 73,700,885 params). Both counts are DISTINCT parameters — never `sum(v.numel() for v in state_dict.values())`, which double-counts the tied per-layer Smolgen generator 16 times and reports 78.8M for the production net. `tests/test_param_count.py` re-measures both and fails if this line drifts.
